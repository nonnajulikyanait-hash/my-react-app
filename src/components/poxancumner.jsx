...
function Poxancumner({ setActiveTab }) {
  // Սլաքով բացվող-փակվող հարցերի state-ը
  const [openIndex, setOpenIndex] = useState(null);
  const [activeCategory, setActiveCategory] = useState('Փոխանցումներ');
  ...
  const handleCategoryClick = (cat) => {
    ...
    // Եթե սեղմում են «Պարտատոմսեր», տեղափոխում ենք պարտատոմսերի էջ (Harcer)
    if (cat === 'Պարտատոմսեր') {
      if (setActiveTab) {
        setActiveTab('harcer');
      }
  ...
  // Փոխանցումների հարցերի և պատասխանների տվյալները
  const questionsList = [
    {
      q: "Ի՞նչ տարբերակներով կարող եմ կատարել փոխանցումներ:",
      a: "Եթե ունեք EvocaTOUCH հավելվածը, ապա կարող եք առանց բանկ մոտենալու կատարել փոխանցումներ ինչպես ՀՀ տարածքում, այնպես էլ ՀՀ տարածքից դուրս:"
    },
    {
      q: "SWIFT փոխանցումներ կատարելիս իմ գումարն ամբողջությա՞մբ է տեղ հասնում:",
      a: "SWIFT փոխանցումներ կատարելիս պետք է ընտրել՝ ցանկանում եք միջնորդավճարն ամբողջությամբ վճարել, որպեսզի գումարը առանց այլ միջնորդավճարների հասնի հասցեատիրջը (OUR), թե ցանկանում եք մինիմալ միջնորդավճար վճարել, ինչի պարագայում յուրաքանչյուր միջնորդ բանկ պահում է կատարում փոխանցվող գումարից (BEN):"
    },
    {
      q: "Կարո՞ղ եմ գումար փոխանցել ոչ աշխատանքային ժամերի:",
      a: "Եթե ունեք EvocaTOUCH հավելված, կարող եք կատարել փոխանցումներ ցանկացած ժամի, ցանկացած վայրից:\nEvocabank-ի հաշիվների միջև փոխանցումը կատարվում է մի քանի րոպեում, իսկ այլ բանկ փոխանցումները կկատարվեն հաջորդ աշխատանքային օրվա ընթացքում:"
    }
  ];
